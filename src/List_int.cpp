...
// 请在此完成6个函数（Create，NumPersons，ShowList，Insert，Append，FreeList）的定义

void Tele::Create(const Person& r)
{
	head = new Person[1000];
	*head = r;
}

void Tele::Append(const Person& r)
{
	Person *temp=NULL;
...
void Tele::ShowList() const
{
	Person* p;
	for (p = head; p!= NULL; p = p->next)
		cout << "[ 姓名：" << p->name << " 性别：" << p->sex << " 年龄：" << p->age << " 电话号码：" << p->tel << " ]" << endl;
}


/* 提示：这是重载运算符 <<，使之能够对链表（Person *）类型进行输出
...
*/
//ostream & operator<<(ostream &out, const Person *head)// 重载输出运算符（<<），使得能够直接用运算符<<输出整条链表
...
*/
//Person * Tele::Locate(string n, int &num, bool newsearch)
//{
//	static Person *p = head;
//	static string name = n;			// 与数据域的类型相同
...
//		k = 0;
//	}
//	for(; p!=NULL && p->name!=name; p=p->next,k++)	// 从链表的"当前"结点起继续查找
//		;
//	temp = p;
//	num = k;
...
//	const Person *p;						// 如果不是为了n的计数。此处可以直接用 outfile << head; 取代下面的 for 循环
